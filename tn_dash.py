...
def run_interface():
    ...
    containers = [
        columns_containers[i].container(border=True, height=700) for i in range(2)
    ]
    ...
    df_rates = process_df(
        st.session_state.start_date, st.session_state.end_date, st.session_state.titulo
    )
    ...
    cl = chart_curves(
        df_rates, st.session_state.toggle_taxa_1, st.session_state.toggle_premio_limpo
    )
    ...
    venc_interp = float(
        columns_painel_2[0]
        .text_input("Vértices de referência (em anos)", "1.5")
        .replace(",", ".")
    )
    ...
    list_interpolations, list_dates = interpolate_rates_for_dates(
        df_rates, venc_interp, st.session_state.start_date, st.session_state.end_date
    )
    ...
    df_interpolation = (
        df_interpolation.groupby("vertice")
        .apply(calculate_basis_point_change)
        .reset_index(drop=True)
    )

    chart_interpolation = chart_lines(
        df_interpolation,
        list_venc_interp,
        st.session_state.toggle_taxa_1,
        em_bps=toggle_bps,
    )
    ...
    if cfg.df_benchmarks.empty:
        ...
        with st.expander("Títulos On-the-Run"):
            if st.session_state.titulo == "DI":
                ...
                st.dataframe(
                    cfg.df_benchmarks[
                        cfg.df_benchmarks["BENCHMARK"].str.contains(
